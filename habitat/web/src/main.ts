...
async function refresh() {
  const session = await api("/auth/me") as { user: User | null };
...
function domainView(view: string) {
  ...
  const maps: Record<string, { eyebrow: string; title: string; description: string; items: string; actions: string }> = {
    ...
    Blueprints: { eyebrow: "BLUEPRINT CATALOG", title: "Blueprints", description: "Available module designs and their build requirements.", items: state.blueprints.map((b) => { const materials = Object.entries(b.inputs ?? {}).filter(([, amount]) => typeof amount === "number" && amount > 0); return `<div class="blueprint-row"><div class="blueprint-title"><strong>${esc(b.displayName || b.blueprintId)}</strong><span class="subtle">${esc(b.blueprintId)} · ${b.ticks ?? "—"} ticks</span></div><div class="materials"><span class="materials-label">Materials</span>${materials.length ? materials.map(([id, amount]) => `<span class="material-chip">${esc(id.replace(/[-_]/g, " "))} × ${Number(amount).toLocaleString()}</span>`).join("") : `<span class="subtle">None required</span>`}</div></div>`; }).join("") || `<div class="empty-state">Catalog is empty. Refresh to load blueprints.</div>`, actions: `<button class="primary" data-command="blueprints">Refresh catalog</button>` },
...
function adminView() { return `<div class="page-title"><div><div class="eyebrow">ADMINISTRATION</div><h1>Admin panel</h1><p class="muted">Manage users and privileged Habitat diagnostics.</p></div></div>${card("Users", "ACCESS CONTROL", `<div id="admin-users" class="empty-state">Loading users…</div>`)}${card("Diagnostics", "PRIVILEGED TOOLS", commands.filter((c) => c.group === "Debug" || c.group === "Settings").map((c) => `<div class="setting-row"><div><strong>${c.title}</strong><span class="subtle">${c.description}</span></div><button class="secondary" data-command="${c.id}">Run</button></div>`).join(""))}`; }
function mountAccountControl() { const target = document.querySelector(".top-actions"); if (!target || document.querySelector("#account-menu")) return; const button = document.querySelector<HTMLButtonElement>("#account-button"); if (!button) { const signIn = document.createElement("button"); signIn.className = "account-button"; signIn.id = "account-button"; signIn.textContent = currentUser?.username ?? "Sign in"; target.append(signIn); } document.querySelector("#account-button")?.addEventListener("click", () => { if (!currentUser) return showAuth(); const menu = document.createElement("div"); menu.id = "account-menu"; menu.className = "account-menu"; menu.innerHTML = `<strong>${esc(currentUser.username)}</strong><span>${currentUser.role}</span>${currentUser.role === "admin" ? `<button id="admin-link">Admin panel</button>` : ""}<button id="signout">Sign out</button>`; document.body.append(menu); document.querySelector("#admin-link")?.addEventListener("click", () => { menu.remove(); activeView = "Admin"; render(); loadAdminUsers(); }); document.querySelector("#signout")?.addEventListener("click", async () => { await api("/auth/logout", { method: "POST" }); currentUser = null; menu.remove(); render(); }); }); }
async function loadAdminUsers() { const node = document.querySelector<HTMLDivElement>("#admin-users"); if (!node || currentUser?.role !== "admin") return; try { const users = await api("/admin/users") as User[]; node.className = "user-list"; node.innerHTML = users.map((user) => `<div class="setting-row"><div><strong>${esc(user.username)}</strong><span class="subtle">Joined ${new Date(user.createdAt).toLocaleString()}</span></div><select data-role-user="${esc(user.username)}"><option value="user" ${user.role === "user" ? "selected" : ""}>User</option><option value="admin" ${user.role === "admin" ? "selected" : ""}>Admin</option></select></div>`).join(""); node.querySelectorAll<HTMLSelectElement>("[data-role-user]").forEach((select) => select.addEventListener("change", async () => { await api(`/admin/users/${encodeURIComponent(select.dataset.roleUser!)}/role`, { method: "POST", body: JSON.stringify({ role: select.value }) }); toast("Role updated"); })); } catch (error) { node.textContent = error instanceof Error ? error.message : String(error); } }
function showAuth() { const dialog = document.createElement("div"); dialog.className = "modal-backdrop"; dialog.innerHTML = `<form class="modal" id="auth-form"><div class="eyebrow">HABITAT ACCOUNT</div><h2>Choose a username</h2><p class="muted">No password is required. This identifies your local UI session.</p><label>Username<input name="username" minlength="2" maxlength="32" required placeholder="user"></label><div class="modal-actions"><button type="button" class="secondary" id="auth-cancel">Cancel</button><button class="primary">Sign up / sign in</button></div><pre class="result" id="auth-result"></pre></form>`; document.body.append(dialog); dialog.querySelector("#auth-cancel")!.addEventListener("click", () => dialog.remove()); dialog.querySelector("form")!.addEventListener("submit", async (event) => { event.preventDefault(); const username = String(new FormData(event.currentTarget as HTMLFormElement).get("username") ?? ""); const result = dialog.querySelector("#auth-result")!; try { let response: { user: User }; try { response = await api("/auth/login", { method: "POST", body: JSON.stringify({ username }) }) as { user: User }; } catch { response = await api("/auth/signup", { method: "POST", body: JSON.stringify({ username }) }) as { user: User }; } currentUser = response.user; dialog.remove(); render(); } catch (error) { result.textContent = error instanceof Error ? error.message : String(error); } }); }

function render() {
  const nav = [{ id: "Overview", icon: "grid" }, { id: "Habitat", icon: "habitat" }, { id: "Modules", icon: "modules" }, { id: "Construction", icon: "construction" }, { id: "Inventory", icon: "inventory" }, { id: "Blueprints", icon: "blueprint" }, { id: "Resources", icon: "resources" }, { id: "Solar & Power", icon: "power" }, { id: "Settings", icon: "settings" }];
  ...
  bindEvents();
  mountAccountControl();
  if (activeView === "Admin") loadAdminUsers();
}
